...
The plot for Group 11 (the group with the strongest correlation) shows a clear positive relationship: higher grades in `Celestial Navigation and Astronomy` are associated with higher grades in `History of the Arcane`, confirming the strong correlation computed previously.
...
We use a  boxplot  to study the differences in final grades between groups.  

This representation highlights potential variations in performance across groups.
...
grades_mod <- grades_long |>
  inner_join(course_modules, by = "course")

module_avgs <- grades_mod |>
  summarise(
    module_avg = mean(avg_grade, na.rm = TRUE),
    .by = c(id, group, module)
  )

cond1 <- grades_wide2 |>
  rowwise() |>
  mutate(no_low_course = all(c_across(-c(id, group, sex)) >= 5)) |>
  ungroup() |>
  select(id, no_low_course)

cond2 <- module_avgs |>
  summarise(all_modules_ok = all(module_avg >= 10), .by = id)

pass_df <- final_grades_df |>
  inner_join(cond1, by = "id") |>
  inner_join(cond2, by = "id") |>
  mutate(
    pass = no_low_course & all_modules_ok
  ) |>
...
course_failures <- grades_wide2 |>
  ...
  filter(avg_grade < 5) |>
  summarise(n_fail = n(), .by = course)

worst_course <- course_failures |>
  slice_max(n_fail, n = 1)
